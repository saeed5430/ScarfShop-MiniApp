...
type AccountInfo = {
  ...
  account: {
    ...
    bale_user_id: string | null;
    bale_phone_masked: string | null;
...
export const BalePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = (await api("/api/admin/bale/status")) as AccountInfo;
        if (cancelled) return;
        setInfo(res);
  ...
  const startConnect = async () => {
    setLoading(true);
    try {
      await api("/api/admin/bale/connect/start", {
        method: "POST",
  ...
  const submitCode = async () => {
    setLoading(true);
    try {
      const res = (await api("/api/admin/bale/connect/code", {
        method: "POST",
  ...
  const submitPassword = async () => {
    setLoading(true);
    try {
      await api("/api/admin/bale/connect/password", {
        method: "POST",
  ...
  const cancelConnect = async () => {
    try {
      await api("/api/admin/bale/connect/cancel", { method: "POST" });
      setStep(0);
      setCode("");
      setPassword("");
      setNeedPassword(false);
  ...
  const disconnect = async () => {
    setLoading(true);
    try {
      await api("/api/admin/bale/disconnect", { method: "POST" });
  ...
  const toggleSending = async (checked: boolean) => {
    try {
      await api("/api/admin/bale/settings", {
        method: "PATCH",
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      {contextHolder}
      <Typography.Title level={3}>ارسال شخصی بله</Typography.Title>
      <Alert
        type="info"
        showIcon
        style={{ marginBottom: 16 }}
        description="حساب شخصی بله خود را متصل کنید تا فاکتور، توضیحات صوتی و پیام‌های تأیید پرداخت از حساب شخصی شما برای خریداران ارسال شود. نشست به‌صورت رمزنگاری‌شده ذخیره می‌شود."
      />
        ...
        <Card title="وضعیت اتصال" style={{ marginBottom: 16 }}>
          <Descriptions column={1} size="small">
            ...
            <Descriptions.Item label="حساب">
              {info.account?.username || info.account?.bale_phone_masked || "—"}
            </Descriptions.Item>
      ...
      <Card title={connected ? "اتصال برقرار است" : "اتصال حساب بله"} loading={!info} style={{ marginBottom: 16 }}>
        {!connected && (
          <>
            <Steps
              size="small"
